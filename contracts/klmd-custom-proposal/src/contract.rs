...
use cosmwasm_std::{
    ...
    StdResult, Storage, WasmMsg, StdError
};
...
use crate::{
    ...
    query::{ProposalResponse, VoteInfo, VoteListResponse, VoteResponse, AllowedAddressesResponse},
    state::{
        get_deposit_msg, get_return_deposit_msg, Ballot, Config, BALLOTS, CONFIG, PROPOSALS,
        PROPOSAL_COUNT, PROPOSAL_HOOKS, VOTE_HOOKS, ALLOWED_ADDRESSES
    },
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::AllowedAddresses {} => query_allowed_addresses(deps)
    }
}
...
pub fn query_allowed_addresses(deps: Deps) -> StdResult<Binary> {
    let allowed_addresses = ALLOWED_ADDRESSES.load(deps.storage)?;
    to_binary(&AllowedAddressesResponse { allowed_addresses })
}
