...
use cosmwasm_std::{
    ...
    Response, StdResult, Storage, SubMsg, WasmMsg, StdError
};
...
use crate::{
    ...
    state::{Ballot, BALLOTS, CONFIG, PROPOSALS, PROPOSAL_COUNT, PROPOSAL_HOOKS, VOTE_HOOKS, ALLOWED_ADDRESSES},
};
...
pub fn instantiate(
    ...
    let mut allowed_addresses: Vec<Addr> = Vec::new();
    allowed_addresses.push(dao.clone());
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::AllowedPropose {
            title,
            description,
            msgs,
            proposer,
        } => execute_allowed_propose(deps, env, info.sender, title, description, msgs, proposer),
        ...
        ExecuteMsg::RegisterAllowedAddress { address } => {
            execute_register_allowed_address(deps, env, info, address)
        },
        ExecuteMsg::DeregisterAllowedAddress { address } => {
            execute_deregister_allowed_address(deps, env, info, address)
        },
    }
}

pub fn execute_allowed_propose(
    deps: DepsMut,
    env: Env,
    sender: Addr,
    title: String,
    ...
    is_allowed_address(&deps, &sender);
...
pub fn execute_propose(
    ...
    println!("Address {}", proposer.as_ref().unwrap().to_string());
    is_allowed_address(&deps, &Addr::unchecked(proposer.as_ref().unwrap().to_string()))?;
    // Determine the appropriate proposer. If this is coming from our
    // pre-propose module, it must be specified. Otherwise, the
    // proposer should not be specified.
    let proposer = match (proposer, &proposal_creation_policy) {
...
pub fn execute_register_allowed_address(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
    address: String,
    ...
    let config = CONFIG.load(deps.storage)?;
    println!("Config: {}",config.dao);
    println!("Sender: {}",info.sender);
    if config.dao != info.sender {
        // Only DAO can register an allowed address
        return Err(ContractError::Unauthorized {});
    }

    let mut allowed_addresses = ALLOWED_ADDRESSES.load(deps.storage)?;
    let validated_address = deps.api.addr_validate(&address)?;

    // verifies address existence
    if allowed_addresses.contains(&validated_address) {
        return Err(ContractError::Exist {});
    }

    allowed_addresses.push(validated_address);
    ALLOWED_ADDRESSES.save(deps.storage, &allowed_addresses)?;

    Ok(Response::default()
        .add_attribute("action", "register_allowed_address")
        .add_attribute("address", address))
}

pub fn execute_deregister_allowed_address(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
    address: String,
    ...
    //let mut new_allowed_addresses: Vec<Addr> = Vec::new();

    //if allowed_addresses.iter().any(|addr| )
    // keeps elements based on a predicate
    allowed_addresses.retain(|addr| addr != &validated_address);
    ALLOWED_ADDRESSES.save(deps.storage, &allowed_addresses)?;
...
pub fn is_allowed_address(deps: &DepsMut, address: &Addr) -> StdResult<()> {
    println!("Addr: {}", address);
    let allowed_addresses = ALLOWED_ADDRESSES.load(deps.storage)?;
    if allowed_addresses.contains(&address){
        return Ok(());
    }
    return Err(StdError::generic_err("Address not allowed"));
...
mod testing {
    use super::*;
    use cosmwasm_std::{testing::{mock_dependencies, mock_env, mock_info}, Addr, from_binary, Uint128, Decimal};
    use cwd_voting::{
    ...
    use crate::{
        testing::{
            instantiate::{
                get_pre_propose_info, instantiate_with_cw4_groups_governance, get_default_non_token_dao_proposal_module_instantiate
            },
            queries::{
                query_single_proposal_module
            },
            execute::{
                make_allowed_proposal
            }
        }
    };
    ...
    fn register_allowed_addr() {
        ...
        // DAO registers a new allowed address 
        let info = mock_info("addr0000", &[]);
        let msg = ExecuteMsg::RegisterAllowedAddress { address: "addr0002".to_string() };
        let _res = execute(deps.as_mut(), env.clone(), info.clone(), msg).unwrap();

        // verify successful registration
        let res = is_allowed_address(&deps.as_mut(), &Addr::unchecked("addr0002".to_string()));
        assert_eq!(res, Ok(()));

        // General address registers a new allowed address
        let info = mock_info("addr00010", &[]);
        let msg = ExecuteMsg::RegisterAllowedAddress { address: "addr0003".to_string() };
        let err = execute(deps.as_mut(), env.clone(), info.clone(), msg).unwrap_err();
        assert!(matches!(err, ContractError::Unauthorized{}));

        //verify unsuccesful registration
        let res = is_allowed_address(&deps.as_mut(), &Addr::unchecked("addr0003".to_string()));
        assert_eq!(res, Err(StdError::generic_err("Address not allowed")));

    }

    #[test]
    fn deregister_allowed_addr() {
        let mut deps = mock_dependencies();
        let mut app = App::default();
        ...
        // deregister an allowed address
        let msg = ExecuteMsg::DeregisterAllowedAddress { address: "addr0002".to_string() };
        let _res = execute(deps.as_mut(), env.clone(), info.clone(), msg).unwrap();
        let res = is_allowed_address(&deps.as_mut(), &Addr::unchecked("addr0002".to_string()));
        assert_eq!(res, Err(StdError::generic_err("Address not allowed")));

        // deregister a not allowed address
        let msg = ExecuteMsg::DeregisterAllowedAddress { address: "addr0004".to_string() };
        let err = execute(deps.as_mut(), env.clone(), info.clone(), msg).unwrap_err();
        assert!(matches!(err, ContractError::NotExist {}));

        // General address deregisters an allowed address
        let msg = ExecuteMsg::RegisterAllowedAddress { address: "addr0005".to_string() };
        let _res = execute(deps.as_mut(), env.clone(), info.clone(), msg).unwrap();
        let res = is_allowed_address(&deps.as_mut(), &Addr::unchecked("addr0005".to_string()));
        assert_eq!(res, Ok(()));

        let info = mock_info("addr0009", &[]);
        let msg = ExecuteMsg::DeregisterAllowedAddress { address: "addr0005".to_string() };
        let err = execute(deps.as_mut(), env.clone(), info.clone(), msg).unwrap_err();
        assert!(matches!(err, ContractError::Unauthorized{}));
    }
    /*
    #[test]
    fn make_allowed_propose() {
        let CREATOR_ADDR: &str = "creator";
        let not_allowed_addr: &str = "addr0002";
        let mut deps = mock_dependencies();
        let mut app = App::default();
        ...
        app.execute_contract(
            ...
            proposal_module,
            &ExecuteMsg::RegisterAllowedAddress {
                address: not_allowed_addr.to_string(),
            },
            &[],
        ).unwrap();
        ...
        let proposal_module = query_single_proposal_module(&app, &core_addr);
        app.execute_contract(
                Addr::unchecked(not_allowed_addr),
                proposal_module,
                &ExecuteMsg::AllowedPropose {
