...
use crate::{
    ...
    testing::{
        ...
        execute::{
            ...
            vote_on_proposal, vote_on_proposal_should_fail, register_allowed_address, deregister_allowed_address, make_allowed_proposal
        },
        instantiate::{
...
fn setup_test(messages: Vec<CosmosMsg>) -> CommonTest {
    ...
    // register allowed address
    register_allowed_address(&mut app, &proposal_module, CREATOR_ADDR);
...
fn test_simple_proposal_cw4_voting() {
    ...
    // register an allowed address
    register_allowed_address(&mut app, &proposal_module, "addr0002");
    let id = make_allowed_proposal(&mut app, &proposal_module, "addr0002", vec![]);
    ...
    let expected = SingleChoiceProposal {
        title: "title".to_string(),
        description: "description".to_string(),
        proposer: Addr::unchecked("addr0002"),
        start_height: current_block.height,
        expiration: Duration::Time(604800).after(&current_block),
        min_voting_period: None,
        threshold: Threshold::ThresholdQuorum {
    ...
    assert_eq!(deposit_response.proposer, Addr::unchecked("addr0002"));
...
#[test]
#[should_panic(
    expected = "Error parsing into type cwd_voting_cw4::msg::QueryMsg: unknown variant `token_contract`"
)]
...
fn test_update_pre_propose_module() {
    ...
    let proposal_id = make_proposal(
        ...
        vec![WasmMsg::Execute {
            ...
            msg: to_binary(&cwd_pre_propose_single::ExecuteMsg::Withdraw { denom: None }).unwrap(),
            funds: vec![],
        }
        .into()],
    );
