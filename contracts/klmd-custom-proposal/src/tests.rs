...
use crate::{
    ...
    proposal::Proposal,
    query::{ProposalListResponse, ProposalResponse, VoteInfo, VoteResponse, AllowedAddressesResponse},
...
#[test]
fn test_propose_with_deregistered_address() {
    let mut app = App::default();
    let govmod_id = app.store_code(single_proposal_contract());
    ...
    // Deregister allowed address
    app.execute_contract(
        governance_addr.clone(),
        govmod_single.clone(),
        &ExecuteMsg::DeregisterAllowedAddress {
            address: CREATOR_ADDR.to_string(),
        },
        &[],
    ).unwrap();

    // Create a new proposal with a deregistered address.
...
#[test]
fn test_query_allowed_addresses() {
    let mut app = App::default();
    let govmod_id = app.store_code(single_proposal_contract());
    let gov_addr = instantiate_with_cw20_balances_governance(
        &mut app,
    ...
    // Query allowed addresses
    let allowed_addresses: AllowedAddressesResponse = app
        .wrap()
        .query_wasm_smart(
            govmod.clone(),
            &QueryMsg::AllowedAddresses {
            },
        )
        .unwrap();
    ...
    // Test after removing an address
    app.execute_contract(
        gov_addr.clone(),
        govmod.clone(),
        &ExecuteMsg::DeregisterAllowedAddress {
